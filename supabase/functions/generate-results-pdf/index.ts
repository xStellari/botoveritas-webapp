...
function positionRank(posRaw: string) {
  ...
  // handle common variants: "vp-internal", "vp - internal", "vice president - internal", etc.
  if (
  ...
  // PRO variants
  if (pos === "pro" || pos.includes("p.r.o") || pos.includes("public") && pos.includes("relations")) return 7;

  // Fallback: unknown positions go last, alphabetical handled by caller
  return 999;
}
...
function drawPageNumber(page: any, font: any, idx: number, total: number) {
...
serve(async (req) => {
  ...
  try {
    ...
    const logo_url =
      (body?.logo_url as string | undefined) ||
    ...
    let signatories: Signatory[] = Array.isArray(body?.signatories)
      ? (body.signatories as Signatory[]).filter((s) => s?.label && s?.name)
      : [];

    // Back-compat (if you still send prepared_by/noted_by)
    const prepared_by = (body?.prepared_by as string | undefined) || "";
    const prepared_by_title =
      (body?.prepared_by_title as string | undefined) || "Prepared by";
    const noted_by = (body?.noted_by as string | undefined) || "";
    const noted_by_title =
      (body?.noted_by_title as string | undefined) || "Noted by";
    ...
    const network =
      ...
      "Polygon (Proof-of-Vote via NFT)";
    const explorer_base =
      (body?.explorer_base as string | undefined) ||
      "https://polygonscan.com/tx/";
    const tx_hashes = Array.isArray(body?.tx_hashes)
      ? (body.tx_hashes as string[]).filter(Boolean).slice(0, 20)
      : [];
    const contract_address = (body?.contract_address as string | undefined) ||
      "";
    const nft_collection =
      (body?.nft_collection as string | undefined) ||
      "BotoVeritas Proof-of-Vote";
