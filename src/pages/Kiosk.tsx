...
const Kiosk = () => {
  ...
  // ⭐ Capture kiosk_id from URL (?kiosk_id=A)
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const kioskId = params.get("kiosk_id");

    if (kioskId) {
      console.log("KIOSK SET:", kioskId);
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-primary/10 via-background to-secondary/10 p-6">
      ...
      <Card className="w-full max-w-3xl border-2 border-primary/30 bg-card/95 backdrop-blur-xl p-12 shadow-2xl animate-fade-in">
        ...
        <div className="flex flex-col items-center text-center mb-10">
          <img src={feuLogo} alt="FEU Alabang" className="h-28 w-auto mb-6 drop-shadow-lg" />
          ...
          <p className="text-muted-foreground mt-2 text-base max-w-md">
            Authenticate using your <strong>RFID Student ID</strong> and <strong>Facial Recognition</strong> to begin voting.
          </p>

          <div className="flex gap-2 mt-4">
            <Badge variant="outline" className="border-primary/40 text-primary bg-primary/10">Secure</Badge>
            <Badge variant="outline" className="border-secondary/40 text-secondary bg-secondary/10">Private</Badge>
            <Badge variant="outline" className="border-amber-500/40 text-amber-600 bg-amber-50">Instant</Badge>
          </div>
        </div>
        ...
        <div className="mt-8 flex items-center justify-center gap-2 text-muted-foreground text-sm">
          ...
          <span>Your identity is encrypted and never stored outside the election system.</span>
        </div>
